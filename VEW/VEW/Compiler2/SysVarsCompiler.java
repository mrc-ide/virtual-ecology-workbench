        ...
        for (int i=0; i<pms.length; i++) {
          if (!pms[i].getAttribute("type").equals(ParticleManager2.RULE_MAINTAIN)) {
            ...
            if (pms[i].getAttribute("alltime").equals("true")) PW.print("ALL_TIME,0,0");
              ...
              PW.print(pms[i].getAttribute("start")+"L,"+pms[i].getAttribute("end")+"L,");
...
//      PW.println("    final double L_v = 2.5E6 - (2.3*T_0);                           // Heat  of vaporisation.");
      PW.println("    final double q_A = Kernel.W.Rel_Humidity;                       // Relative  humidity from  ERA40  data");
      PW.println("    final double q_S = 6.112*Math.exp((17.67*T_0)/(T_0+243.5));     // Saturation-specific humidity  at sea surface temp");
      PW.println("    final double U_vec = Kernel.W.WSpeed;                           // From  ERA40  data");
      PW.println("");
//      PW.println("    final double slhf = -(C_E * rho * L_v * U_vec * (0.98 * (q_S - q_A)));");
      ...
      PW.println("    final double T_A = Kernel.W.Air_Temp;      // Air temperature (K) from  ERA40");
      PW.println("");
      ...
      PW.println("    final double Ch_d = Kernel.W.Cloud_Cover;         // Cloud-cover (0..1) from ERA40");
