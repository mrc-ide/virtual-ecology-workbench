...
public class BlackBox {
  ...
  public static void CompileGetVar(XMLTag model, String eq, PrintWriter PW, XMLTag equation, XMLTag function, XMLTag group, boolean fgUpdate) {
    ...
    // Deal with internal pools
      ...
      if ((v == null) && (group.getTag("predator") != null) && (model.getTag("closure") != null) && (group.getValue("predator").equals("true"))) v = model.getTag("closure")
          .getTagWhere("predator", "name", group.getValue("name")).getTagWhere("*", "name", eq);
      if (v != null) {
        ...
        if (((tag.equals("variable")) || (tag.equals("fgvar")))) PW.print(vname + "[1]");
          ...
          if ((vname.equals("Ingestion")) || (vname.equals("IngestedCells"))) PW.print("IngestedCells[_ingIndex]");
          else {
  ...
  public static void CompileExpression(XMLTag model, String eq, PrintWriter PW, XMLTag equation, XMLTag function, XMLTag group, boolean fgUpdate) {
    ...
    else if (StringTools.chomp(eq, "\\temperatureAt{")) CompileUnaryFunc(model, StringTools.spit(eq, "\\temperatureAt{"), "Utils.getTemperature", PW, equation, function, group,
        fgUpdate);
    ...
    else if (StringTools.chomp(eq, "\\ln{")) CompileUnary(model, StringTools.spit(eq, "\\ln{"), "Math.log", PW, equation, function, group, fgUpdate);
    ...
    while ((eq.indexOf("someVariety") >= 0) || (eq.indexOf("allVariety") >= 0) || (eq.indexOf("noVariety") >= 0) || (eq.indexOf("varietymul") >= 0)
      ...
      if (!s.startsWith("variety")) {
        ...
        if (linkName.equals("Ingestion")) {
          ...
          if (eq.indexOf("{IngestedCells}") >= 0) PW.println("      int _ingIndex = SysVars.getIngIDFor(params." + linkName + "_types[_" + linkName + "],params." + linkName
              + "_status[_" + linkName + "]);");
        }
      ...
      PW.print("    ");
      if (!s.startsWith("variety")) eq = eq.substring(0, i - 1) + "\\" + "text{_varResult" + VarietyLabelCount + "}" + eq.substring(2 + s.length() + i + exp.length());
  ...
  public static void debugVariableDump(XMLTag model, PrintWriter PW, XMLTag equation, XMLTag function, XMLTag group, boolean fgUpdate, String vars) {
    ...
    int i = 0;
    PW.println("      System.out.println(\"NaN Error, " + group.getValue("name") + ":" + function.getValue("name") + ":" + equation.getValue("name")
        + ", ID=\"+id+\", stage=\"+_stageNames[_CurrentStage]+\" Timestep=\"+Kernel.timeSteps);");
  ...
  public static void writeDeathReason(XMLTag model, PrintWriter PW, String fgName, String s) {
    ...
    for (int i = 0; i < validSpecies.length; i++) {
      ...
      while ((speciesNo < model.getTag("species").getTags("species").length) && (!model.getTag("species").getTags("species")[speciesNo].getAttribute("name").equals(species)))
        speciesNo++;
  ...
  public static void writeSingleLayerFoodIngestion(PrintWriter PW, String speciesName, XMLTag model, String foodset, String[] paramlist, String spaces, XMLTag eq, XMLTag function,
      XMLTag group) {
  ...
  public static void writeMultiLayerFoodIngestion(PrintWriter PW, String speciesName, XMLTag model, String foodset, String[] paramlist, String spaces, XMLTag eq, XMLTag function,
      XMLTag group) {
    XMLTag[] foods = model.getTag("foodsets").getTagWhere("foodset", "@name", speciesName + " : " + foodset).getTags("food");
    for (int i = 0; i < foods.length; i++) {
      PW.println(spaces + "_" + foodset + "=" + i + ";");
  ...
  public static void CompileStat(XMLTag model, String eq, PrintWriter PW, XMLTag equation, XMLTag function, XMLTag group, ArrayList deathReasons, boolean fgUpdate) {
    ...
    if (StringTools.chomp(eq, "\\while{")) CompileWhile(model, StringTools.spit(eq, "\\while{"), PW, equation, function, group, fgUpdate);
      ...
      PW.println("        if (Logging.getLineage()) Logging.writeCreateToLineage(X" + countCreates + ".params._type, Kernel.myTime, id, X" + countCreates + ".id,_CurrentStage,X"
          + countCreates + "._CurrentStage);");
      PW.println("        if (Logging.getLifespan()) Logging.writeCreateToLifespan(X" + countCreates + ".params._type, 0, Kernel.myTime, id, X" + countCreates + ".id, X"
          + countCreates + ".c[1], _STAGE_" + StageName + ");");
      PW.println("      }");
      PW.println("    }");
      countCreates++;
      ...
      if (debugMode) {
        ...
        CompileExpression(model, paramlist[0], PW, equation, function, group, fgUpdate);
        PW.println(")){");
        PW.println("        System.out.println(\"Warning: TS=\"+Kernel.timeSteps+\" Release  NaN of " + chemName
            + " by \"+_stageNames[_CurrentStage]+\" \"+name()+\", id=\"+id+\", in " + function.getValue("name") + " : " + equation.getValue("name") + "\");");
        ...
        PW.println("        System.out.println(\"Warning: TS=\"+Kernel.timeSteps+\" Release  \"+_amount+\" of " + chemName
            + " by \"+_stageNames[_CurrentStage]+\" \"+name()+\", id=\"+id+\", in " + function.getValue("name") + " : " + equation.getValue("name") + "\");");
        debugVariableDump(model, PW, equation, function, group, fgUpdate, paramlist[0]);
  ...
  public static boolean[] getChemPoolUsage(XMLTag fg, XMLTag model) {
    ...
    for (int i = 0; i < chemUse.length; i++) {
      ...
      if (chems[i].getValue("pigment").equals("true")) chemUse[i] = true;
        ...
        XMLTag[] dists = model.getTag("initialplankton").getTags("distribution");
        for (int j=0; j<dists.length; j++) {
          if (model.getTag("species").getTagWhere("species","@name",dists[j].getAttribute("species")).getAttribute("fg").equals(fg.getValue("name"))) {
            XMLTag initVar = dists[j].getTagWhere("var","@name",chems[i].getValue("name")+"$Pool");
            if (initVar!=null) {
              if ((Double.parseDouble(initVar.getAttribute("val"))>0) || (Double.parseDouble(initVar.getAttribute("rnd"))>0)) {
                chemUse[i]=true;
                j=dists.length;
              }
            }
  ...
  private static void CompileFunctionalGroup(XMLTag model, XMLTag fg, ArrayList deathReasons) {
    ...
    for (int i = 0; i < specs.length; i++)
      if (StringTools.nonSpace(specs[i].getAttribute("fg")).equals(fgName)) FGPW.println("    _" + StringTools.nonSpace(specs[i].getAttribute("name")) + "Params.set"
          + StringTools.nonSpace(specs[i].getAttribute("name")) + "Params();");
    ...
    for (int i = 0; i < chems.length; i++) {
      ...
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("  public double _request_" + chem + "=0.0;");
      if (chemInUse(model, fg, chem, "release")) FGPW.println("  public static double _release_" + chem + "=0.0;");
    }
    ...
    for (int i = 0; i < funcs.length; i++)
      CompileFunction(model, funcs[i], FGPW, fg, deathReasons, true);
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("      if (_request_" + chem + ">0) _b1.request_" + chem + "+=_request_" + chem + ";");
    }
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("          if (_request_" + chem + ">0) _b.request_" + chem + "+=_request_" + chem + "*_prop;");
    }
    ...
    for (int j = 0; j < chems.length; j++) {
      ...
      if (chemInUse(model, fg, chemName, "uptake")) FGPW.println("      if ((_request_" + chemName + ">0) && (_b1.deplete_" + chemName + "<1)) _request_" + chemName
          + "*=_b1.deplete_" + chemName + ";");
    }
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
    ...
    for (int j = 0; j < chems.length; j++) {
      final String chemName = StringTools.nonSpace(chems[j].getValue("name"));
      if (chemInUse(model, fg, chemName, "uptake")) FGPW.println("      final double _orig_up_" + chemName + " = _request_" + chemName + ";");
    }
    FGPW.println("      for (int _layer=(int)_b1.Depth; _layer<=_b2.Depth; _layer++) {");
    FGPW.println("        BLayer _b = Kernel.W.B_Layer[_layer];");
    FGPW.println("        double _prop = (_b==_b1)?_topProp:(_b==_b2)?_bottomProp:_midProp;");
    ...
    for (int j = 0; j < chems.length; j++) {
      final String chemName = StringTools.nonSpace(chems[j].getValue("name"));
      if (chemInUse(model, fg, chemName, "uptake")) FGPW.println("          if ((_request_" + chemName + ">0) && (_b.deplete_" + chemName + "<1)) _request_" + chemName
          + "-=(_orig_up_" + chemName + "*_prop*(1-_b.deplete_" + chemName + "));");
    }
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
    ...
    vs = fg.getTags("varietyvariable");
    for (int i = 0; i < vs.length; i++) {
      if ((vs[i].getValue("name").indexOf("$") == -1) && (!vs[i].getValue("name").equals("IngestedCells"))) FGPW.println("    for (int _shv=0; _shv<" + vs[i].getValue("name")
          + ".length; _shv++) " + vs[i].getValue("name") + "[_shv][0]=" + vs[i].getValue("name") + "[_shv][1];");
    }
    ...
    for (int i = 0; i < w.length; i++)
      FGPW.println("    " + w[i].getValue("name") + " = 0.0;");
    ...
    if (pchangeNeeded) {
      ...
      for (int i = 0; i < vs.length; i++) {
        if ((vs[i].getValue("name").indexOf("$") == -1) && (!vs[i].getValue("name").equals("IngestedCells"))) {
          ...
          if (hist < 1) hist = 1;
          for (int j = hist; j > 0; j--)
            FGPW.println("        for (int _shv=0; _shv<" + vs[i].getValue("name") + ".length; _shv++) X" + countCreates + "." + vs[i].getValue("name") + "[_shv][" + j + "]=X"
                + countCreates + "." + vs[i].getValue("name") + "[_shv][" + (j - 1) + "];");
        }
      }
      ...
      FGPW.println("        if (((X" + countCreates + ".log) && (SysVars.getStageLog(X" + countCreates + ".getParams()._type,X" + countCreates
          + "._CurrentStage)) || ((log) && (SysVars.getStageLog(params._type, _CurrentStage))))) {");
      FGPW.println("          if (Logging.getLineage()) Logging.writePChangeChildToLineage(X" + countCreates + ".getParams()._type, Kernel.myTime, X" + countCreates
          + ".id, id,_CurrentStage,X" + countCreates + "._CurrentStage);");
      FGPW.println("          if (Logging.getLifespan()) Logging.writePChangeChildToLifespan(X" + countCreates + ".getParams()._type, Kernel.myTime, X" + countCreates
          + ".id, id, X" + countCreates + ".c[1], _i);");
      FGPW.println("        }");
      FGPW.println("      }");
      FGPW.println("    }");
    ...
    for (int i = 0; i < vs.length; i++) {
      if ((vs[i].getValue("name").indexOf("$") == -1) && (!vs[i].getValue("name").equals("IngestedCells"))) {
        ...
        if (hist < 1) hist = 1;
        for (int j = hist; j > 0; j--)
          FGPW.println("    for (int _shv=0; _shv<" + vs[i].getValue("name") + ".length; _shv++) " + vs[i].getValue("name") + "[_shv][" + j + "]=" + vs[i].getValue("name")
              + "[_shv][" + (j - 1) + "];");
      }
    }
    ...
    for (int i = 0; i < speciesTags.length; i++) {
      ...
      if ((fg.getTag("predator") != null) && (model.getTag("closure") != null) && (fg.getValue("predator").equals("true"))) {
        ...
        for (int j = 0; j < vs.length; j++)
          FGPW.println("      " + StringTools.nonSpace(vs[j].getValue("name")) + " = " + vs[j].getValue("value") + ";");
      }
        ...
        if (predatorSpeciesName.equals(speciesName)) {
          ...
          if (foods.length > 0) {
            ...
            for (int pn = 0; pn < paramCount; pn++)
              paramNames[pn] = paramTags0[pn].getAttribute("name");
  ...
  private static void CompileFGSnapshotSupport(XMLTag model, PrintWriter FGPW, XMLTag fg) {
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("      snapshot.writeDouble(_request_" + chem + ");");
    }
    ...
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
          final String foodFgName = model.getTag("species").getTags("species")[i].getAttribute("fg");
          final XMLTag foodFg = model.getTagWhere("functionalgroup", "name", foodFgName);
          FGPW.println("      snapshot.writeDouble(_requested_" + StringTools.nonSpace(specs[i].getAttribute("name")) + "_"
    ...
    XMLTag[] speciesTags = model.getTag("species").getTags("species");
    for (int i = 0; i < speciesTags.length; i++)
      if (StringTools.nonSpace(speciesTags[i].getAttribute("fg")).equals(StringTools.nonSpace(fgName))) FGPW.println("      if (specid==" + i + ") X.params = _"
          + StringTools.nonSpace(speciesTags[i].getAttribute("name")) + "Params;");
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("      X._request_" + chem + "=snapshot.readDouble();");
    }
    ...
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
          final String foodFgName = model.getTag("species").getTags("species")[i].getAttribute("fg");
          final XMLTag foodFg = model.getTagWhere("functionalgroup", "name", foodFgName);
          FGPW.println("      X._requested_" + StringTools.nonSpace(specs[i].getAttribute("name")) + "_" + StringTools.nonSpace(foodFg.getTags("stage")[j].getValue("name"))
              + "=snapshot.readDouble();");
        }
      }
    }
  ...
  private static void compileMemoryManagement(PrintWriter FGPW, XMLTag fg, XMLTag model) {
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chemName = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemsInUse[i]) FGPW.println("    if (_" + chemName + "_Pool_Old!=_fg2._" + chemName + "_Pool_Old) _" + chemName + "_Pool_Old = ((_OurCells*_" + chemName
          + "_Pool_Old)+(_TheirCells*_fg2._" + chemName + "_Pool_Old))/(_totalCells);");
    }
    ...
    for (int i = 0; i < varvars.length; i++) {
      ...
      if (!name.equals("IngestedCells")) {
        ...
        if (hist < 1) hist = 1;
        for (int j = 0; j <= hist; j++)
          FGPW.println("        " + name + "[_" + link + "][" + j + "]=((" + name + "[_" + link + "][" + j + "]*_OurCells)+(_fg2." + name + "[_" + link + "][" + j
              + "]*_TheirCells))/_totalCells;");
        FGPW.println("    }");
      }
    }
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("        if (_request_" + chem + ">0) _b1.request_" + chem + "=Math.max(0,_b1.request_" + chem + "-_request_" + chem
          + ");");
    }
    for (int j = 0; j < chemsInUse.length; j++) {
      if (chemsInUse[j]) {
        ...
        FGPW.println("        _b1.particulate_chem_ss[" + cid + "][params._type][_CurrentStage]=Math.max(0,_b1.particulate_chem_ss[" + cid + "][params._type][_CurrentStage]-_"
            + chemName + "_c);");
      }
    }
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("            if (_request_" + chem + ">0) _b.request_" + chem + "=Math.max(0,_b.request_" + chem + "-(_request_"
          + chem + "*_prop));");
    }
    ...
    for (int j = 0; j < chemsInUse.length; j++) {
      if (chemsInUse[j]) {
        ...
        FGPW.println("            _b.particulate_chem_ss[" + cid + "][params._type][_CurrentStage]=Math.max(0,_b.particulate_chem_ss[" + cid + "][params._type][_CurrentStage]-_"
            + chemName + "_c);");
      }
    }
    FGPW.println("          }");
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("        if (_fg2._request_" + chem + ">0) _b1.request_" + chem + "=Math.max(0,_b1.request_" + chem
          + "-_fg2._request_" + chem + ");");
    }
    for (int j = 0; j < chemsInUse.length; j++) {
      if (chemsInUse[j]) {
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("            if (_fg2._request_" + chem + ">0) _b.request_" + chem + "=Math.max(0,_b.request_" + chem
          + "-(_fg2._request_" + chem + "*_prop));");
    }
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("    _request_" + chem + "+=_fg2._request_" + chem + "; // _request is already *c");
    }
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("    X._request_" + chem + "=0;");
    }
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("    _request_" + chem + "/=_no;");
    }
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
    ...
    for (int i = 0; i < chems.length; i++) {
      final String chem = StringTools.nonSpace(chems[i].getValue("name"));
      if (chemInUse(model, fg, chem, "uptake")) FGPW.println("    _request_" + chem + "=_fg._request_" + chem + ";");
    }
    for (int i = 0; i < ingNeeded.length; i++) {
      for (int j = 0; j < ingNeeded[i].length; j++) {
        if (ingNeeded[i][j]) {
  ...
  private static void CompileModel(XMLTag model, ArrayList deathReasons) {
    ...
    for (int i = 0; i < fgs.length; i++)
      CompileFunctionalGroup(model, fgs[i], deathReasons);
    XMLTag[] chs = model.getTags("chemical");
    for (int i = 0; i < chs.length; i++)
      CompileChemical(model, chs[i], i);
  }
