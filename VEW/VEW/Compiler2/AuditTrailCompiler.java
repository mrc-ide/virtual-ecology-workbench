    ...
    if (theVars.length>0) {
      ...
      for (int i=0; i<theVars.length; i++) {
        ...
        if (scope.equals(OutputDialog2.SCOPE_AT_AGENT)) {
          ...
          if (mode==LOGGING) PW.println("      Logging.writeFGReal("+auditSpeciesNo+",blayer.SpeciesConcentration_old["+targetSpeciesNo+"]["+targetStageNo+"]);");
          else varID=writeVarEntry(PW, varName,"real",varID);
          ...
          if (chemType.equals(OutputDialog2.DEF_SOLN)) {
            ...
            if ((var.getAttribute("species")!=null) && (var.getAttribute("stage")!=null)) {
              if (mode==LOGGING) {
                ...
                PW.println("        Logging.writeFGReal("+auditSpeciesNo+",blayer.particulate_chem_ss_old["+chemName+".ID_"+chemName+"]["+specNo+"]["+stageNo+"]);");
              } else varID=writeVarEntry(PW,varName,"real",varID); 
            } else {
              final String chemName = var.getAttribute("chem");
              if (mode==LOGGING) PW.println("      Logging.writeFGReal("+auditSpeciesNo+",blayer.particulate_chem["+chemName+".ID_"+chemName+"]);");
              else varID=writeVarEntry(PW,varName,"real",varID);
            }
